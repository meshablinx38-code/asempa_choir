...
class FirestoreService {
  ...
  Future<SessionModel> createSession({required String createdBy,
    ...
    final session = SessionModel(
      id: id, qrCode: qrCode, createdAt: now,
      expiresAt: now.add(validity), isActive: true,
      createdBy: createdBy,
  ...
  Stream<SessionModel?> streamActiveSession() => _db
    .collection('sessions').where('isActive', isEqualTo: true).limit(1)
    .snapshots()
  ...
  Future<bool> checkIn({required String uid, required String qrCode}) async {
    ...
    batch.set(attRef, AttendanceRecord(
      id: attRef.id, uid: uid, sessionId: session.id,
      date: DateTime.now(), present: true).toFirestore());
  ...
  // ── Quiet Time (collection: quiet_time_posts) ─────────────────────────────
  ...
  Stream<List<QuietTimePost>> streamQuietTimePosts({int limit = 20}) => _db
    .collection('quiet_time_posts').orderBy('date', descending: true).limit(limit)
  ...
  Stream<DevotionalModel?> streamTodayDevotional() {
    ...
    return _db.collection('devotionals')
      ...
      .where('date', isLessThan: Timestamp.fromDate(end)).limit(1)
      .snapshots()
  ...
  Future<Map<String, int>> getAdminStats() async {
    ...
    final complete = users.docs.where((d) {
      final data = d.data();
      return data['fullName'] != null && data['email'] != null &&
...
[System.IO.File]::WriteAllText("lib\services\firestore_service.dart", $fix, [System.Text.Encoding]::UTF8)
Write-Host "firestore_service.dart fixed!" -ForegroundColor Green
